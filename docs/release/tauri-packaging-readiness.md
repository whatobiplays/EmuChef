...
| Runtime quarantine checks | `npm run check:rust-runtime` | Runs app-local runtime checks, including no-Python-runtime, no-Python-editor-API, no-Python-fixture-regeneration, and no-PySide-runtime scans for active editor paths, active Rust/Tauri checks, and default distribution metadata. | Release builds, real packaged GUI behavior, signing, notarization, updater readiness, fixture refresh, or full repo Python deletion. |
...
- [`apps/config-editor/scripts/check-no-python-fixture-regeneration.mjs`](../../apps/config-editor/scripts/check-no-python-fixture-regeneration.mjs) verifies that normal Rust/Tauri active checks do not invoke Python fixture or golden regeneration.
...
- [`docs/python-fixture-golden-ownership.md`](../python-fixture-golden-ownership.md) classifies checked-in fixture/golden ownership and labels remaining Python regeneration commands as dev-only/reference-only.
