...
| Error and warning compatibility | Rust covers selected planner result, warning/error shape, and focused diagnostics. P8P adds pure/test-backed detected-device profile mismatch warning construction, and P8Q appends that warning in a crate-private fake/test-backed planning-result helper. Full CLI stderr/stdout, route-level detected-device profile mismatch warnings, operation replay failures, exit codes, and broader planner diagnostics are not proven. `detected_device_profile_mismatch_warning_not_cut_over` remains blocked until route-level evidence exists. |
