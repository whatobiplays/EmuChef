...
| `plan` | Python planner/CLI owns default planning result emission, concise/verbose/`--output` behavior, and exit-code behavior. | Rust planner is an internal fixture-backed module that emits Python-shaped `PlanningResult`/`ExecutionPlan` values for selected tests. P8A adds only an explicit developer bridge, `emuchef plan --planner-backend rust-shadow --rust-planner-bin <path>`, that forwards raw binds to the supplied shadow binary and passes through Rust JSON stdout/stderr/exit code. P8B adds a dev-only matrix smoke for that explicit Python CLI route's invocation path. P8C guards the bridge as JSON/text passthrough, not Python CLI output compatibility; `--output` and `--verbose` are rejected for `rust-shadow`. None of these paths is exposed as a protocol request, Tauri command, production/default CLI command, Cargo fallback, or replacement planner. | Full authored catalog loading, device profile/plan behavior, draft operations, bindings, diagnostics, output YAML/text, CLI `--output`/verbose parity, exit-code semantics, and default-route ownership. | Keep planner declarative; do not add runtime behavior to planning parity work. |
...
The P8C contract keeps the bridge as Rust stdout/stderr/exit-code passthrough and
explicitly not Python CLI YAML, concise summary, `--output`, or verbose output
compatibility.
