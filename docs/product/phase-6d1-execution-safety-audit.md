...
| Operation timeouts | Partially implemented | Passive device probing and the explicit root check use bounded process waits; root timeout is sanitized as `timedOut`. Artifact HTTP operations use their existing bounded resolver policy. | `ProcessCommandRunner::run_bounded` in `device_probe.rs`; `AdbCommandExecutor::run_with_timeout` and `probe_root` in `executor/adb.rs`; root timeout regressions. | General ADB install, copy, shell, launch, force-stop, verification, and sidecar request/response operations are unbounded. Adding deadlines requires child-process ownership and outcome policy, so it is deferred rather than patched with unsafe thread timeouts. |
| Sidecar crash or protocol loss | Partially implemented | Broken request I/O, EOF, malformed JSON, response-ID mismatch, and structurally invalid success or error envelopes mark the runtime failed and stop the unusable child. Every later request in that process generation returns the stable `runtime_session_lost` code. Tauri translates that proof of global loss to `execution_unavailable` and invalidates every execution/launch mapping plus runtime-derived review, device-fact, and root-qualification authority. An `unknown_execution` backend error remains mapping-local. The UI states that a lost real-device outcome may be partial and cannot be resumed or rolled back. | `SidecarClient::request`, `fatal_protocol_loss_stops_the_process_and_marks_the_runtime_failed`, `malformed_success_envelope_invalidates_the_runtime_session`, `malformed_error_envelope_invalidates_the_runtime_session_persistently`, and `valid_backend_error_does_not_invalidate_the_runtime_session` in `sidecar.rs`; `execution_session_loss`, `invalidate_lost_runtime_authority`, and `lost_runtime_session_resets_all_execution_mappings` in Tauri `execution.rs`; unavailable workflow tests. | A live but permanently hung sidecar is not detectable without the deferred IPC timeout design. |
...
1. Define one bounded timeout and child-process ownership model for sidecar IPC
   and every production ADB install, copy, shell, launch, force-stop, and
...
typecheck, lint, build, and diff commands. The implementation uses deterministic
host tests and does not claim new physical-device or release qualification.
