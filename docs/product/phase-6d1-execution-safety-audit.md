...
7. Phase 6D.3 adds bounded, line-anchored ADB transport classification across
   ...
   sanitized guidance. Same-serial replacement, root revocation, and physical
...
1. Qualify the bounded timeout and child-process ownership model against
   ...
   supported devices.
2. Design mid-run safe-boundary revalidation for target identity, same-serial
   replacement, qualification context, and root authority.
3. Define low-storage and host-sleep behavior, including whether the outcome is
   terminal failed or explicitly indeterminate.
4. Run the physical interruption matrix for cancellation timing, disconnect,
...
typecheck, lint, build, and diff commands. Phase 6D.2 and Phase 6D.3 use
deterministic host tests and do not claim new physical-device, packaged-GUI, or
