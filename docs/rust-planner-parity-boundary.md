...
- `crates/emuchef-rust-backend/src/planner_tests.rs`: Rust unit tests comparing
  ...
  the P7A fixture inventory/parsing guard.
...
| Dependency validation | `src/emuchef/planner/dependencies.py`; `src/emuchef/io/validation.py` | `phase6l_catalog_validation.rs`; planner dependency tests | Rust covers selected recipe and step dependency ordering/cycle diagnostics. | Match Python planner diagnostics, ordering, and failure behavior for planner-facing dependency cases. |
...
| Diagnostics/error model | `src/emuchef/domain/errors.py`; `src/emuchef/planner/*`; `src/emuchef/io/validation.py` | Required binding missing, dependency cycle, catalog validation tests | Rust covers selected error/status shapes. | Port or retire all planner-facing diagnostics, details fields, CLI output, and exit-code expectations. |
