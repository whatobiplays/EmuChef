...
| Device plan | Device profile | Selected recipes | Private Rust planner status | Required planner-only bindings | Current gap |
| --- | --- | --- | --- | --- | --- |
...
| `authored/recipes/app.xaniteog.install.yaml` | `app.xaniteog.install` | Success through Rust domain model. | Unbound synthetic context emits deterministic `binding_missing` for `app.xaniteog.install/xaniteog_apk`; placeholder `.apk` binding emits successfully in the bound synthetic corpus context. P7I/P7J verifies supplied binding ingestion for a checked-in device plan that selects this recipe. P7L records the checked-in `ayaneo.pocket_s2.base` repo-plan context as a current Rust planner gap even when a temp `.apk` binding is supplied, because the selected RetroArch flow fails first on non-emitted app-data copy dependencies. | Manual selected ref plus synthetic fixture context in P7H; checked-in repo device-plan/profile selected-ref ingestion in P7I/P7J/P7L. | Planner-only APK binding does not validate APK payloads; current checked-in plan success is blocked by RetroArch dependency/capability behavior, not APK payload validation. | `planner_tests::authored_corpus_unbound_required_inputs_emit_classified_errors`; `planner_tests::authored_corpus_supported_synthetic_context_emits_execution_plan`; `planner_tests::repo_device_plan_ingestion_accepts_supplied_bindings_without_applying_metadata_overrides`; `planner_tests::repo_plan_e2e_checked_in_context_gaps_are_classified_without_expanding_scope` |
| `authored/recipes/feature.copy_bios.yaml` | `feature.copy_bios` | Success through Rust domain model. | Unbound synthetic context emits deterministic `binding_missing` for `feature.copy_bios/bios_source_dir`; placeholder directory binding emits successfully in the bound synthetic corpus context. P7I/P7J verifies supplied binding ingestion for checked-in device plans that select this recipe. P7L records checked-in repo-plan contexts that select this recipe as current Rust planner gaps even when a temp BIOS directory binding is supplied, because the selected RetroArch flow fails first on non-emitted app-data copy dependencies. | Manual selected ref plus synthetic fixture context in P7H; checked-in repo device-plan/profile selected-ref ingestion in P7I/P7J/P7L. | Planner-only BIOS binding does not inspect BIOS contents; current checked-in plan success is blocked by RetroArch dependency/capability behavior, not BIOS payload validation. | `planner_tests::authored_corpus_unbound_required_inputs_emit_classified_errors`; `planner_tests::authored_corpus_supported_synthetic_context_emits_execution_plan`; `planner_tests::repo_device_plan_ingestion_accepts_supplied_bindings_without_applying_metadata_overrides`; `planner_tests::repo_plan_e2e_checked_in_context_gaps_are_classified_without_expanding_scope` |
