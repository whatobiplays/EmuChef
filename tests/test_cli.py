...
class CliTests(unittest.TestCase):
    ...
    def test_plan_default_requires_explicit_binary_path_for_rust_route(self) -> None:
        stdout = StringIO()
        stderr = StringIO()
        with (
    ...
    def test_plan_default_output_writes_yaml_and_prints_summary_from_rust_route(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
    ...
    def test_plan_default_rejects_python_only_and_explicit_rust_wrapper_flags_before_subprocess_work(self) -> None:
        cases = [
            (["--debug"], "--debug"),
            (["--adb", "/does/not/exist"], "--adb"),
            (["--ops", "ops.yaml"], "--ops"),
            (["--serial", "SERIAL"], "--serial"),
            (["--rust-shadow-output", "passthrough"], "--rust-shadow-output"),
            (["--rust-detected-facts-json", "facts.json"], "--rust-detected-facts-json"),
            (["--rust-probe-adb-getprop", "--rust-adb-path", "adb", "--rust-serial", "SERIAL"], "--rust-probe-adb-getprop"),
            (["--rust-adb-path", "adb"], "--rust-adb-path"),
            (["--rust-serial", "SERIAL"], "--rust-serial"),
        ]
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            for extra_args, expected_option in cases:
                with self.subTest(extra_args=extra_args):
                    ...
                    self.assertEqual(rc, 1)
                    self.assertEqual(stdout.getvalue(), "")
                    self.assertIn(expected_option, stderr.getvalue())
                    build_session.assert_not_called()
                    resolve_adb.assert_not_called()
                    detect_device.assert_not_called()
                    run.assert_not_called()

    def test_plan_rust_production_equivalent_requires_explicit_binary_path(self) -> None:
        stdout = StringIO()
        stderr = StringIO()
        with (
            patch("emuchef.cli._build_session") as build_session,
    ...
    def test_plan_rust_production_equivalent_rejects_rust_shadow_output_before_subprocess_work(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            stdout = StringIO()
            stderr = StringIO()
    ...
    def test_plan_rust_production_equivalent_forwards_detected_facts_fixture_exact_string_without_python_planner_or_adb(
        self,
    ) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            fixture_arg = "~/fixtures/$DEVICE facts.json"
            completed = subprocess.CompletedProcess(
                args=[],
                returncode=0,
                stdout=self._rust_shadow_planning_result_json(),
        ...
        self.assertNotIn("kind: planning_result", stdout.getvalue())
        self.assertEqual(stderr.getvalue(), "")
        build_session.assert_not_called()
        resolve_adb.assert_not_called()
        detect_device.assert_not_called()
    ...
    def test_plan_rust_production_equivalent_forwards_live_probe_exact_strings_without_python_planner_or_adb(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
    ...
    def test_plan_rust_production_equivalent_rejects_incomplete_or_conflicting_live_probe_wrapper_args_before_work(
        self,
    ) -> None:
        cases = [
            (
    ...
    def test_plan_rust_production_equivalent_verbose_emits_structured_yaml(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            completed = subprocess.CompletedProcess(
                args=[],
    ...
    def test_plan_rust_production_equivalent_output_writes_yaml_and_prints_summary(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            output_path = Path(tmp) / "planning-result.yaml"
            completed = subprocess.CompletedProcess(
    ...
    def test_plan_rust_production_equivalent_rejects_python_only_flags_before_subprocess_work(self) -> None:
        unsupported_args = [
            ["--debug"],
            ["--adb", "/does/not/exist"],
            ["--ops", "ops.yaml"],
        ...
        with TemporaryDirectory() as tmp:
            ...
            for extra_args in unsupported_args:
                with self.subTest(extra_args=extra_args):
                    ...
                    self.assertEqual(rc, 1)
                    self.assertEqual(stdout.getvalue(), "")
                    self.assertIn("is not supported with --planner-backend rust-production-equivalent", stderr.getvalue())
    ...
    def test_plan_rust_experimental_rejects_raw_detected_facts_fixture_flag_at_python_cli_layer(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            stdout = StringIO()
            stderr = StringIO()
    ...
    def test_plan_rust_experimental_forwards_detected_facts_fixture_exact_string_without_python_planner_or_adb(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            fixture_arg = "~/fixtures/$DEVICE facts.json"
            completed = subprocess.CompletedProcess(
    ...
    def test_plan_python_backend_rejects_rust_detected_facts_fixture_before_planning_work(self) -> None:
        stdout = StringIO()
        stderr = StringIO()
        with (
            patch("emuchef.cli._build_session") as build_session,
        ...
        self.assertEqual(rc, 1)
        self.assertEqual(stdout.getvalue(), "")
        self.assertIn("--rust-detected-facts-json is only valid with --planner-backend rust-experimental", stderr.getvalue())
        build_session.assert_not_called()
        resolve_adb.assert_not_called()
        detect_device.assert_not_called()
        run.assert_not_called()

    def test_plan_rust_shadow_rejects_rust_detected_facts_fixture_before_planning_work(self) -> None:
        with TemporaryDirectory() as tmp:
            shadow_bin = self._shadow_bin(tmp)
            stdout = StringIO()
            stderr = StringIO()
